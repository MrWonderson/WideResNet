...
class Train(object):
    ...
    def test(self, test_image_array, test_label_array):
        ...
        correct_prediction = tf.equal(tf.cast(tf.argmax(predictions, 1), 'int32'), self.test_label_placeholder)
        ...
        for step in range(num_batches):
            ...
            batch_prediction_array, accu = sess.run([predictions, accuracy],
            ...
            accuracy_overall = accu_sum / num_batches
        ...
        if remain_images != 0:
            ...
            batch_prediction_array, accu = sess.run(predictions, accuracy, feed_dict={
                self.test_image_placeholder: test_image_batch})

            prediction_array = np.concatenate((prediction_array, batch_prediction_array))
            accu_sum += accu
            accuracy_overall = accu_sum / (num_batches + 1)

        return prediction_array, accuracy_overall
    ...
    def loss(self, logits, labels):
        ...
        :param labels: 2D tensor with shape [batch_size, NUM_CLASSES]
    ...
    def generate_vali_batch(self, vali_data, vali_label, vali_batch_size):
        ...
        :param vali_label: D numpy array
    ...
    def generate_augment_train_batch(self, train_data, train_labels, train_batch_size):
        ...
        :param train_labels: D numpy array
    ...
    def full_validation(self, loss, top1_error, session, vali_data, vali_labels, batch_data,
        ...
        :param vali_data: 4D numpy array
        :param vali_labels: D numpy array
        :param batch_data: 4D numpy array. training batch to feed dict and fetch the weights
        :param batch_label: D numpy array. training labels to feed the dict
        :return: float, float
        '''
...
# predictions, accuracy = train.test(test_image_array, test_label_array)

# print("The accuracy on test set is %.5f" % accuracy)